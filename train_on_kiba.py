...
#############################
import os
os.environ["CUDA_LAUNCH_DEVICES"] = "1"
...
class NHGNN(nn.Module):
    def __init__(self, embedding_dim, lstm_dim, hidden_dim, dropout_rate,
        ...
        self.iMOLD = MyModel(args=args, config=cfg).to(device)
        # SMILES
    ...
    def forward(self, data, reset=False):
        ...
        smiles = torch.zeros(batchsize, seq_len).to('cuda:1').long()
        protein = torch.zeros(batchsize, tar_len).to('cuda:1').long()
        smiles_lengths = []
        protein_lengths = []
        ...
        # 修改部分 GNN网络
        # gout = self.hgin(data)
        gout,c_graph_feat, s_graph_feat, cmt_loss, loss_reg = self.iMOLD(data)
        pre_y_final=gout * self.theta + out.view(-1, 1) * (1 - self.theta)

        loss_fn = torch.nn.MSELoss()

        # 这个过程确保数据中的 NaN 值被适当地处理
        reg_loss = loss_fn(pre_y_final.float(), data.y.view(-1, 1).float().to(device)).float()
        mix_f = self.iMOLD.mix_cs_proj(c_graph_feat, s_graph_feat)
        inv_loss = simsiam_loss(c_graph_feat, mix_f)
        loss = reg_loss  +  0.1*inv_loss +  loss_reg + 0.1 * cmt_loss

        return pre_y_final,loss
...
def args_parser():
    ...
    # VQ
    parser.add_argument("--num_e", default=12000, type=int)
...
cfg={'random_seed': 123, 'num_workers': 1, 'model': {'dim_hidden': 300, 'dim_ffn': 300, 'dropout_rate': 0.5, 'model_layer': 3, 'global_pool': 'mean', 'model_name': 'GIN'}, 'dataset': {'dataloader_name': 'BaseDataLoader', 'generate': False, 'dataset_name': 'GOODZINC', 'domain': 'scaffold', 'shift_type': 'covariate'}, 'train': {'train_bs': 32, 'val_bs': 32, 'test_bs': 32}}
cfg = munchify(cfg)
cfg.device = device
...
for sm in smiles:
    ...
    if not smiles_idx.__contains__(sm):
        ...
        print('built model...')
        model_1 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                  alpha=alpha, n_heads=n_heads).to(device)
        model_2 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                  alpha=alpha, n_heads=n_heads).to(device)
        model_3 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                  alpha=alpha, n_heads=n_heads).to(device)
        model_4 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                  alpha=alpha, n_heads=n_heads).to(device)
        model_42 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                      alpha=alpha, n_heads=n_heads).to(device)
        model_6 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                        alpha=alpha, n_heads=n_heads).to(device)
        model_7= NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                        alpha=alpha, n_heads=n_heads).to(device)
        model_8 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                        alpha=alpha, n_heads=n_heads).to(device)
        model_9 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                        alpha=alpha, n_heads=n_heads).to(device)
        model_10 = NHGNN(embedding_dim=embedding_dim, lstm_dim=lstm_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate,
                         alpha=alpha, n_heads=n_heads).to(device)

        load_model_path = 'kiba_pretrain_model_cold_'+scene+'_'+i+'.model'
        model_file_name = 'model_kiba_NHGNN_iMOLD_cold_'+scene+'_codebook_num_16000_1_'+i+'.model'
        result_file_name = 'result_kiba_NHGNN_iMOLD_cold_'+scene+'_codebook_num_16000_1_'+i+'.csv'
        result_file_name_1 =  'result_kiba_NHGNN_iMOLD_cold_'+scene+'_codebook_num_16000_1_withname'+i+'.csv'
        print('creat dataset...')
        ...
        dataset_train = DTADataset(root='../Data/' + dataset_name, path='../Data/' +  'kiba_cold_'+scene+'_train_'+i+'.csv', smiles_emb=smiles_emb,
                             target_emb=target_emb, smiles_idx=smiles_idx, smiles_graph=smiles_graph, target_graph=target_graph,
                             smiles_len=smiles_len, target_len=target_len)

        dataset_test = DTADataset(root='../Data/' + dataset_name, path='../Data/' +  'kiba_cold_'+scene+'_test_'+i+'.csv', smiles_emb=smiles_emb,
                             target_emb=target_emb, smiles_idx=smiles_idx, smiles_graph=smiles_graph, target_graph=target_graph,
                             smiles_len=smiles_len, target_len=target_len)

        dataset_valid = DTADataset(root='../Data/' + dataset_name, path='../Data/' +  'kiba_cold_'+scene+'_valid_'+i+'.csv', smiles_emb=smiles_emb,
                             target_emb=target_emb, smiles_idx=smiles_idx, smiles_graph=smiles_graph, target_graph=target_graph,
                             smiles_len=smiles_len, target_len=target_len)
        ...
        if load_model_path is not None:
            ...
            save_model = torch.load(load_model_path,map_location='cuda:1')
        ...
        def reset_feature(dataset, model):
            ...
            with torch.no_grad():
                ...
                for data in tqdm(dataloader):
                    ...
                    #是不是没有1/2（CLS_t+CLS_S),只取了CLS_S
                    for i in range(start, min(len(dataset), start + batch_size)):
                        dataset.data[i].x = torch.cat([pro[i - start, 1:tar_len[i - start] - 1], sm[i - start, idx[i - start]],
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if epoch % reset_epoch == 0:
                ...
                reset_feature(dataset_train, model)
                reset_feature(dataset_test, model)
                reset_feature(dataset_valid, model)
            ...
            G, P ,sm,target,true,predict = predicting(model, val_loader)
            val1 = get_mse(G, P)
            # writer = SummaryWriter('runs/davis_cold_Target_withoutpretrain_3')
            # writer.add_scalar('val_mse',val1,epoch)
            # writer.close()
            RMSE = rmse(G, P)
            # pearson = pearson(G, P)
            # CI = ci(G, P)
            ...
            if val1 < best_mse:
                best_mse = val1
                best_epoch = epoch + 1
                ret = [rmse(G, P), mse(G, P), pearson(G, P), spearman(G, P), ci(G, P), get_rm2(G, P),get_aupr(G,P)]
                ...
                calculate_metrics(G, P)
            if epoch == 200 or epoch == 30 or epoch == 50 or epoch == 100 or epoch == 150 or epoch == 250 or epoch == 300 or epoch == 350:
                if model_file_name is not None:
                    print('load_model...', model_file_name)
                    save_model = torch.load(model_file_name)
                    model_dict = model.state_dict()
                    state_dict = {k: v for k, v in save_model.items() if k in model_dict.keys()}
                    model_dict.update(state_dict)
                    model.load_state_dict(model_dict)
                G1, P1 ,sm,target,true,predict= predicting(model, test_loader)
                val2 = get_mse(G1, P1)

                import csv

                ret = [epoch, rmse(G1, P1), mse(G1, P1), pearson(G1, P1), spearman(G1, P1), ci(G1, P1), get_rm2(G1, P1),get_aupr(G1,P1)]
                header = ['epoch', 'rmse', 'mse', 'pearson', 'spearman', 'ci', 'rm2','aupr']
            ...
            if epoch ==50:
                ...
                with open(result_file_name_1, 'w', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(['Drug Name', 'Target Name','True Affinity', 'Predicted Affinity'])
                    for sm,target, true_affinity, pred_affinity in zip(sm, target,true,predict):
                        writer.writerow([sm,target, true_affinity, pred_affinity])
